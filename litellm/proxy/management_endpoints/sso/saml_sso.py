...
class SAMLAuthHandler:
    ...
    def _response_in_response_to(auth: "OneLogin_Saml2_Auth") -> Optional[str]:
        ...
        value = cast(Optional[str], auth.get_last_response_in_response_to())
        if value:
            return value
        xml = cast(Optional[bytes], auth.get_last_response_xml())
        if not xml:
            return None
        root = OneLogin_Saml2_XML.to_etree(xml)
        for node in OneLogin_Saml2_XML.query(root, "//saml:SubjectConfirmationData[@InResponseTo]"):
            irt = cast(Optional[str], node.get("InResponseTo"))
            if irt:
                return irt
        return None
